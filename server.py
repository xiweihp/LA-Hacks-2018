...
def get_function(data):
    ...
    if not nostring and not allstring:
        return 'error: cannot do operation between string and int/double'
...
@app.route('/api/function_result',methods = ['GET','POST'])
def function_result():
    temp = request.get_json()
    datastring = temp['param']
    datadict = {}
    ...
    for x in range (0,number):
        datadict['object%d'%(x)] = datalist[3+x]
    return jsonify(get_function(json.dumps(datadict)))
